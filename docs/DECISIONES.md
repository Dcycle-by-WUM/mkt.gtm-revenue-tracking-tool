...
| 15 | **Precedencia de contacto en deals multi-contacto** | ⏳ Pendiente | Auditoría 20-jul (§5, caso Stadler Rail AG - CSRD): `fetchDealAssociations` guarda solo 1 contacto por deal (`r.to?.[0]`, el primero que devuelve la API), pero un deal puede tener varios contactos asociados con canal distinto (Stadler tiene 3: OFFLINE, Paid Search y Other Campaigns). Si el guardado no es el champion, el fallback de la decisión #14 no ve el canal paid real y el deal cae en Otros. Propuesta sin confirmar: traer todos los contactos asociados e priorizar el de `analytics_source` PAID_SOCIAL/PAID_SEARCH al atribuir canal. Pendiente decidir con Davide antes de tocar `deal_attribution` (afecta históricos de todos los países/meses). |
