-- Fallback de país por EMPRESA para deals sin contacto asociado (Davide,
-- 09-jul, follow-up de la auditoría 0017). Casos reales: Brockton Everlast
-- y Gavazzi Tessuti Tecnici (International Pipeline, 14.000 € y 11.000 €)
-- no tienen contacto asociado al deal, así que `deal_attribution` nunca
-- podía leer su país aunque la EMPRESA ya lo tuviera (Brockton → country
-- 'UK' en `accounts` desde antes de esta migración).
--
-- Precedencia final: mapa por pipeline (AE→ES) > país de la campaña >
-- país del contacto > país de la empresa > 'INTL'.
...
-- Vistas KPI idénticas a 0017 (heredan el fallback nuevo vía deal_attribution).
