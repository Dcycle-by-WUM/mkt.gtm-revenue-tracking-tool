-- El flag lead_source='Inbound' manda sobre la fuente OFFLINE (Davide,
-- 20-jul). Son dos campos distintos en HubSpot:
--   · lead_source = 'Inbound'      → marca explícita "esto es inbound".
--   · hs_analytics_source = OFFLINE → fuente de tráfico "sin canal online".
-- Un contacto puede ser AMBOS (p. ej. Sergio Gracia en el deal SOCOTEC:
-- Inbound, pero traffic source OFFLINE porque entró sin cookie/UTM).
--
-- La decisión #16 (excluir OFFLINE) miraba SOLO la fuente de tráfico, así
-- que tiraba deals que están marcados Inbound explícitamente — pisando la
-- #14. Corrección (regla final de Davide): "si el lead_source dice inbound,
-- el deal es inbound; si además la fuente es OFFLINE, cuéntalo como Otros".
--
-- Qué cambia:
--   · DEALS: si tras resolver canal (contacto paid > fuente del deal >
--     fuente del contacto) no hay canal, pero el contacto asociado es
--     lead_source='Inbound' y anterior al deal → canal 'Otros' (antes se
--     excluía). SOCOTEC → Spain + Otros. Ubesol sigue fuera (su contacto es
--     Outbound, no Inbound).
--   · LEADS: un contacto con fuente OFFLINE pero lead_source='Inbound'
--     explícito cuenta como lead 'Otros' (antes se excluía). Los OFFLINE sin
--     Inbound explícito siguen fuera.
--
-- Se mantiene la regla #17 (el contacto debe predatar el deal) también en
-- este rescate. Limitación conocida: el rescate por contacto usa el contacto
-- GUARDADO en el deal (hubspot_contact_id); en deals multi-contacto donde el
-- Inbound no sea el guardado podría no verse (los paid sí se detectan entre
-- todos vía paid_contact_channel).
--
-- Solo-vista: aplicar en el SQL editor de producción; efecto inmediato tras
-- refresh_kpi_views(), sin deploy.
...
with base as (
  select
    ...
    -- canal resuelto, por precedencia:
    --   1. contacto paid entre TODOS los asociados (date-gated en ingesta)
    --   2. fuente propia del deal (null si OFFLINE)
    --   3. fuente del contacto guardado, si predata el deal (null si OFFLINE)
    --   4. contacto Inbound explícito anterior al deal, sin canal → 'Otros'
    -- Si todo es null → deal no-inbound → se excluye (WHERE de abajo).
    coalesce(
      d.paid_contact_channel,
      source_to_channel(d.analytics_source),
      case
        when ct.created_at_hs is not null and d.createdate is not null
          and ct.created_at_hs < d.createdate
        then source_to_channel(ct.analytics_source)
      end,
      case
        when ct.lead_source = 'Inbound'
          and ct.created_at_hs is not null and d.createdate is not null
          and ct.created_at_hs < d.createdate
        then 'Otros'
      end
    )                                           as resolved_channel
...
-- No-paid por mes con desglose de canal. Incluye contactos OFFLINE SOLO si
-- son lead_source='Inbound' explícito (→ canal Otros); el resto de OFFLINE
-- (sin marca inbound) sigue fuera.
create materialized view kpi_organic_by_month as
with organic_contacts as (
  select
    ct.id,
    ct.is_mql,
    coalesce(source_to_channel(ct.analytics_source), 'Otros') as channel,
  ...
  where coalesce(ct.analytics_source, '') not in ('PAID_SOCIAL', 'PAID_SEARCH')
    ...
    and coalesce(ct.email, '') not ilike '%@dcycle.io'
    and (
      source_to_channel(ct.analytics_source) is not null   -- tiene canal (organic/email/otros)
      or ct.lead_source = 'Inbound'                         -- o inbound explícito aunque sea OFFLINE
    )
    and not exists (
