-- Canal paid entre TODOS los contactos asociados a un deal, no solo el
-- primero (decisión #15, auditoría 20-jul — caso Stadler Rail AG - CSRD).
--
-- Problema: `hs_analytics_source` de un deal en HubSpot es, por
-- definición de HubSpot, "el origen del contacto con la actividad MÁS
-- ANTIGUA asociado a este deal" — no el champion, no el contacto que
-- originó la oportunidad. En cuentas con varios contactos (ABM/
-- enterprise), un contacto viejo no-paid puede tapar a un champion real
-- de pago asociado al mismo deal:
--
--   Stadler Rail AG - CSRD: Lino Mesa (OFFLINE, contacto desde 2024) tapa
--   a Nadya Segui (Paid Search, la champion real, contacto de feb-2026).
--   Intentar arreglarlo marcando a Nadya como "primary contact" en
--   HubSpot NO cambia `hs_analytics_source` — ese campo no mira la marca
--   de "primary", solo la actividad más antigua. Verificado en vivo
--   20-jul: tras el intento de arreglo manual, el deal seguía OFFLINE.
--
-- Además la ingesta (`lib/hubspot.ts`) solo guardaba 1 contacto por deal
-- (`fetchDealAssociations` cogía `r.to[0]`, el primero que devolvía la
-- API de HubSpot — sin garantía de que fuera el champion). El fallback de
-- la migración 0020 ("deal OFFLINE → mira el canal del ÚNICO contacto
-- guardado") solo puede acertar si ese contacto es el correcto.
--
-- Fix (este migration + `lib/hubspot.ts`): la ingesta ahora trae TODOS
-- los contactos asociados a cada deal y, si CUALQUIERA de ellos tiene
-- `analytics_source` PAID_SOCIAL/PAID_SEARCH, lo guarda en dos columnas
-- nuevas (`paid_contact_channel`, `paid_contact_id`). La vista
-- `deal_attribution` prioriza esa señal sobre el rollup de HubSpot y
-- sobre el fallback de contacto único de 0020 — y de paso cubre también
-- los deals cuya fuente propia no es OFFLINE/vacía pero tampoco es paid
-- (ORGANIC_SEARCH, DIRECT_TRAFFIC, OTHER_CAMPAIGNS…), que antes nunca
-- llegaban a mirar al contacto.
--
-- Verificado contra HubSpot en vivo (auditoría 20-jul, todo 2026, AE +
-- International): 4 deals con este patrón — Stadler (27.500 €), Familia
-- Torres (30.000 €), Savills (25.000 €), PROSOL (8.750 €) = 91.250 € que
-- pasan de "Otros" a su canal paid real en el próximo sync-crm tras
-- aplicar esta migración. El resto del pipeline "Otros" (52 de los 56
-- deals multi-contacto revisados, más todos los deals de 1 solo
-- contacto) es no-paid genuino — no cambia.
--
-- Aplicar en el proyecto Supabase de producción (`cwcvurrkqwifpngzecxu`)
-- vía SQL editor o `supabase db push` — no hay despliegue automático de
-- migraciones en este repo (ver .github/workflows/ci.yml, solo
-- typecheck+build). El fix solo tiene efecto una vez que corra el
-- siguiente `sync-crm` (cron horario) tras aplicar esta migración Y el
-- deploy del cambio en `lib/hubspot.ts`.

alter table deals add column if not exists paid_contact_channel text;  -- 'LinkedIn' | 'Google' | null
alter table deals add column if not exists paid_contact_id text;       -- qué contacto lo justifica (auditoría / display)
...
select
  ...
  case
    ...
    -- Señal nueva: cualquier contacto asociado (no solo el guardado en
    -- hubspot_contact_id) que sea paid gana, sin importar qué diga el
    -- rollup de HubSpot ni si la fuente propia era OFFLINE/blank/otra.
    when d.paid_contact_channel is not null then d.paid_contact_channel
  ...
  case
    when d.paid_contact_channel is not null then 'contacto'
...
where d.createdate is not null
  and (
    ...
    -- fallback 0021: cualquier contacto asociado es genuinamente paid —
    -- señal más fuerte que "OFFLINE excluye", entra sea cual sea la fuente
    -- propia del deal o si el contacto guardado es Inbound explícito
    or d.paid_contact_channel is not null
    -- fallback 0020: deal OFFLINE pero el (único) contacto guardado es
    -- Inbound explícito y no interno
    or (
...
-- Vistas KPI idénticas a 0020 (heredan el fix vía deal_attribution).
