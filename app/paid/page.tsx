...
import {
  ...
  emptyFilters, emptyMetrics, OVERRIDES_KEY, type CampaignRow, type CountryOverrides,
} from "@/lib/mock-data";
...
// Agrega las filas filtradas por campaña (suma de los meses seleccionados).
...
export default function PaidPage() {
  ...
  const [notes, setNotes] = useLocalState<Record<string, string>>("gtm.paidNotes.v1", {});
  const [filters, setFilters] = useState(emptyFilters);

  const all = applyOverrides(mockCampaigns, overrides);
  const rows = aggregateByCampaign(filterCampaigns(all, filters));
  ...
  return (
    <div>
      <PageHeader
        ...
        subtitle="Canal × campaña con todas las métricas (§7.6). Filtra por país/mes/canal y deja una nota de apuntes por campaña."
      />
      <StatusBanner />
              ...
              <tr key={r.campaign} className="border-t border-[var(--border)]">
                ...
                <td className="px-3 py-2">
                  <input
                    ...
                    onChange={(e) => setNotes({ ...notes, [r.campaign]: e.target.value })}
                    placeholder="Apunte…"
      ...
      <p className="mt-3 text-xs text-[var(--muted)]">
        Las notas se guardan en tu navegador (en producción, en Supabase con autor/fecha).
      </p>
    </div>
  );
}
